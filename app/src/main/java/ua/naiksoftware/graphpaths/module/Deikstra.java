...
public class Deikstra extends Module {
    ...
    private static final String tag = Deikstra.class.getName();
    ...
    private class CellD extends Cell {
        ...
        public String toString() {
            if (passability == Integer.MAX_VALUE) return INFINITY;
            else return String.valueOf(passability);
        }
    ...
    private void findDeikstra(Cell[][] m, int start, int end) {
        ...
        getCanvasView().addString("Path from " + (start + 1) + " to " + (end + 1)
                //+ " equal " + (idx == -1 || row[end].passability == Integer.MAX_VALUE ? INFINITY : row[end].passability));
                + " equal " + row[end]);
        if (idx != -1 && row[end].passability != Integer.MAX_VALUE) {
